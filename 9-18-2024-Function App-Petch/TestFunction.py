...
def load_file():
    ...
    def categorize_item(row, keyword):
        item = row['รายการ']
        expense = row['รายจ่าย']
        income = row['รายรับ']
        #ตรวจรายจ่าย
        if pd.notna(expense) and expense != 0:
            for category, items in keyword.items(): #ตรวจสอบ keyword 
                if any(i in item for i in items): #เช็คว่ารายการตรงกับ keyword ไหม 
                    return category
    ...
    df['หมวดหมู่'] = df.apply(lambda row: categorize_item(row, keyword), axis=1)
