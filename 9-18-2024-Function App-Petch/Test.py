...
def load_keywords():
    ...
    try:
        ...
        for category, keyword_col in [('สถานที่', 'keyword 1'), 
            ...
            if category in df.columns and keyword_col in df.columns:
                keyword_data = df[[category, keyword_col]].dropna()
                for _, row in keyword_data.iterrows():
                    if pd.notna(row[keyword_col]):
                        keywords = [keyword.strip() for keyword in row[keyword_col].split(',')]
                        categories[row[category]] = keywords
    except Exception as e:
