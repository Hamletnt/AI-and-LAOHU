...
root = tk.Tk()
root.title("Laohu")
root.configure(bg="#AF8260")

frame = tk.Frame(root, width=200, height=200)
frame.pack(side="left", anchor="n", padx=10, pady=10)  # ให้อยู่ซ้ายบน (north) พร้อมระยะห่าง
frame.configure(bg="#AF8260")

frame2 = tk.Frame(root)
frame2.place(x=310,y=5, width=1050,height=200)
frame2.configure(bg="#AF8260")

frame3 = tk.Frame(root, width=400, height=100)
frame3.place(x=0, y=183, relwidth=1)  # เพิ่มระยะห่างด้านบนเพื่อให้มีช่องว่าง
frame3.configure(bg="#AF8260")

# โหลดและปรับขนาดรูปภาพ
original_image = Image.open("Laohu-Photoroom.png")  # โหลดรูปภาพ
resized_image = original_image.resize((150, 150), Image.LANCZOS)  # ปรับขนาดรูปภาพ
image = ImageTk.PhotoImage(resized_image)

# สร้าง Label สำหรับรูปภาพ
image_label = Label(frame, image=image)
image_label.pack(side="left")  # วางรูปภาพทางด้านซ้าย
image_label.configure(bg="#982B1C")

# สร้าง Label สำหรับข้อความ "LAOHU"
text_label = tk.Label(frame, text="LAOHU", font=("Georgia", 25, "bold"))
text_label.pack(side="left", padx=5, pady=10)  # วางข้อความทางด้านขวาของรูปภาพ พร้อมระยะห่าง 10 พิกเซล
text_label.configure(bg="#AF8260")

# Create a Notebook for tabbed interface
notebook = ttk.Notebook(frame3, height=558)
notebook.pack(side="bottom", fill=tk.BOTH, pady=(2,2), expand=True)

# Create frames for each tab
full_data_frame = tk.Frame(notebook,height=1000,bg="#AF8260")
filtered_data_frame = tk.Frame(notebook,height=500,bg="#AF8260" )
