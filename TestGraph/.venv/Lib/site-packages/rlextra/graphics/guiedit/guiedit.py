...
def binitmatch(t,bn):
	return t[:2]+t[2:].replace(' ','') == f'\t\t{bn}.__init__(self,width,height,*args,**kw)'

_excNamePat=re.compile(r"^\s*(?:global\s*|)name '(?P<name>[a-zA-Z_]\w*)' is not defined$")
_initPat=re.compile(r"^\tdef __init__\s*\(\s*self\s*,\s*width\s*=\s*([^,]+)\s*,\s*height\s*=\s*([^,]+)\s*,\s*\*args\s*,\s*\*\*kw\s*\)\s*:\s*$")
			...
			if state==0 and t[:5]=='class':
				...
				if t[:2]=='\t\t':
					...
					if _ifmainPat.match(t):
						...
						state = 6
						rcpat=re.compile(rf'^\t\treturn\s+{self.baseName}.getContents\s*\(\s*self\s*\)\s*$')
						edits = self.dedits
					else:
...
class GUIEditor:
	def __init__(self, top):
		...
		class GUIEditorBalloon(Pmw.Balloon):
			def _showBalloon(self, widget, help, isItem, tagOrItem=None):
				Pmw.Balloon._showBalloon(self, widget, help, isItem, tagOrItem)
	...
	def pack(self):
		...
		if self._state=='splash':
			...
			if self.project.className:
				if config.attrList.on:
					...
					if self._attrListSwitchOn:
						pmda._relmin[alName] = 0.05
		...
		if self._attrListSwitchOn:
			self._attrListSwitchOn = False
			root.after(50,self._clean_up_pmda_rel)

	def _clean_up_pmda_rel(self):
		pmda = getattr(self,'pmda',None)
		if pmda:
			alName = 'attrList' if 'attrList' in pmda._relmin else 'pmdah'
			pmda._relmin[alName] = None
			pmda._min[alName] = 1
	...
	def _attrListToggle(self,v):
		...
		if self.project.className:
			self._attrListSwitchOn = v and len(getattr(getattr(self,'pmda',None),'_relsize',[]))<2
			self._state = None
			self.redraw()
	...
	def runGO(self):
		...
		try:
			...
			kwd = dict(
						...
						ioutdir=goValues.setdefault('ioutdir','[callable object]' if hasattr(sample.outDir,'__call__') else sample.outDir),
						balloon=self._balloon,
						isCSV = isCSV,
						)
			if not isCSV:
		...
		try:
			try:
				...
				ioutdir = goValues['ioutdir'] = d._outdir.getvalue()
				if ioutdir!='[callable object]':
					sample.outDir = ioutdir
