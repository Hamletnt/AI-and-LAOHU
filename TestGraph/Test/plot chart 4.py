...
def create_pie_chart(data, labels, title):
    ...
    wedges, texts, autotexts = ax.pie(data, wedgeprops=dict(width=0.5), startangle=-40, colors=colors[:len(labels)],
                                      autopct='%1.1f%%', labeldistance=1.5, pctdistance=0.85)

    # ปรับแต่งข้อความของเปอร์เซ็นต์
    for autotext in autotexts:
        autotext.set_size(8)  # ปรับขนาดของเปอร์เซ็นต์

    # การตั้งค่าลูกศรเชื่อมโยงให้แสดง labels นอกวงกลม
