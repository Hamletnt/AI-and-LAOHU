...
wb = load_workbook('Test/Laohu_MM4.xlsx', data_only=True)
ws = wb.active
...
for keyword in keywords:
    ...
    if column_letter:
        ...
        value = ws.cell(row=2, column=column_number).value  # ดึงค่าจากแถว count_row
...
for keyword in keywords_cost:
    ...
    elif keyword == 'ค่าเช่า ค่าส่วนกลาง นํ้า':
...
if total_invest > 0:
    ...
    for keyword in price_invest:
        if total_cost > 0:
            ...
            percent_invest[keyword] = 0
else:
    percent_invest = {keyword: 0 for keyword in price_invest}
...
if (total_income >0 and total_cost >0):
    ...
    month = ws['B2'].value
    # ตรวจสอบเดือน
    # แปลงสตริงให้เป็น datetime object โดยระบุฟอร์แมตที่ตรงกับข้อมูลในเซลล์ B2
