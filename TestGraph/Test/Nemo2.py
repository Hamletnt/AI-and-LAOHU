...
import numpy as np
import TestNemo2parinya as data
...
wb = load_workbook('Test/Laohu_MM4.xlsx')
ws = wb.active
...
def filter_positive_values(data, labels):
    filtered_data = []
    filtered_labels = []
    for value, label in zip(data, labels):
        if value > 0:  # Keep only positive values
            filtered_data.append(value)
            filtered_labels.append(label)
    return filtered_data, filtered_labels

# Filter data for the pie charts
adjusted_percent_material, adjusted_Category = filter_positive_values(adjusted_percent_material, adjusted_Category)
adjusted_percent_invest, adjusted_invest = filter_positive_values(adjusted_percent_invest, adjusted_invest)
adjusted_percent_total_cost, adjusted_cost = filter_positive_values(adjusted_percent_total_cost, adjusted_cost)
adjusted_percent_sell, adjusted_sell = filter_positive_values(adjusted_percent_sell, adjusted_sell)
