...
def save_as_pdf():
    ...
    with PdfPages(pdf_filename) as pdf:
        ...
        charts_and_tables = {
            ...
            "Invest Cost": (adjusted_percent_invest, adjusted_invest, None, "Invest Cost"),
        ...
        for title, (data, labels, values, chart_title) in charts_and_tables.items():
            ...
            # สร้าง Pie Chart ที่ ax1
            wedges, texts, autotexts = ax1.pie(data, autopct='%1.1f%%', wedgeprops=dict(width=1), startangle=-40, colors=plt.get_cmap('tab20').colors[:len(labels)])
