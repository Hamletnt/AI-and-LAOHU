...
# ฟังก์ชันในการสร้าง Pie Chart แบบ Donut Chart พร้อมเส้นเชื่อมแบบงอและเก็บส่วน 'ไม่เกี่ยว'
def create_pie_chart_with_curved_labels(data, labels, title):
    ...
    # สร้าง Donut Chart โดยใช้ width=0.4 เพื่อทำให้เป็น donut chart
    wedges, texts, autotexts = ax.pie(data, wedgeprops=dict(width=0.4), startangle=90, colors=colors[:len(labels)],
                                      autopct='%1.1f%%', pctdistance=0.85)

    # ปรับแต่งการแสดงเปอร์เซ็นต์
    for autotext in autotexts:
        autotext.set_color('black')
        autotext.set_size(8)

    # กำหนดลูกศรเชื่อมแบบงอ
    bbox_props = dict(boxstyle="round,pad=0.3", fc="w", ec="k", lw=0.72)
    kw = dict(arrowprops=dict(arrowstyle="-|>", connectionstyle="arc3,rad=0.3"),
    ...
    for i, p in enumerate(wedges):
        ...
        connectionstyle = f"arc3,rad={0.5 * np.sign(x)}"  # ปรับรัศมีการงอของเส้น
...
def open_new_window(data, labels, title):
    ...
    fig = create_pie_chart_with_curved_labels(data, labels, title)
...
# Donut Chart ที่ 1
fig1 = create_pie_chart_with_curved_labels(adjusted_percent_material, adjusted_Category, "Material Cost")
...
# Donut Chart ที่ 2
fig2 = create_pie_chart_with_curved_labels(adjusted_percent_invest, adjusted_invest, "Invest Cost")
...
# Donut Chart ที่ 3
fig3 = create_pie_chart_with_curved_labels(adjusted_percent_total_cost, adjusted_cost, "Cost Portion")
...
# Donut Chart ที่ 4
fig4 = create_pie_chart_with_curved_labels(adjusted_percent_sell, adjusted_sell, "Sell Portion")
