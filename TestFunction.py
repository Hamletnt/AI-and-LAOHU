...
browse_button = tk.Button(window, text ='Browse File',width = 10, command=lambda:upload_file()).place(x=400, y=40)
load_button = tk.Button(window, text ='Load File',width = 10, command=lambda:load_file()).place(x=500, y=40)

label_file1= tk.Label(window, text = "File Name --> ")
label_file1.place(x=400, y=80)
label_file2= tk.Label(window, text = "No file Selected")
label_file2.place(x=500, y=80)

# Create LabelFrame and place it
excel_data_frame = LabelFrame(window, text='Excel Data')
excel_data_frame.place(x=100, y=100, height=300, width=800)
