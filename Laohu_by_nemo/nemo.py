    ...
    if file_path:
        try:
            ...
            if 'วันที่' in df.columns:
                df['วันที่'] = pd.to_datetime(df['วันที่'], format='%m/%d/%Y')

            # Optional: Format the date to string if needed
            df['วันที่'] = df['วันที่'].dt.strftime('%m/%d/%Y')
    ...
    for row_id in tree.get_children():
        ...
        if col5_value:
            ...
            if predicted_category == 'เนื้อวัว':
                ...
                category = 'หมู'
            elif predicted_category == 'เนื้อไก่':
                category = 'ไก่'
